...
#### （61）auto关键字和deceltype

auto 关键字进行类型推断
auto 和 auto &，前者定义的变量是非引用的，后者定义的是引用类型
'''
int i=0,c=&i;
auto a=c; // a不是i的引用
auto& a2=c; // a2是i的引用
...
deceltype 选择并返回操作数或表达式的数据类型，表达式不会被执行
'''
decltype(f())  //返回f()的类型，不会调用f()
decltype((f())) //返回类型加个引用，如int f() 返回int &
int* p;
decltype(*p) // 返回 int & 
'''
