 ...
 **虚拟内存实现机制**
 ...
 实现虚拟内存的关键是通过地址转换机制，这个和内存管理机制相关（分页管理，分段管理和段页式管理）通常是通过使用页表或段表来实现。这些表记录了虚拟地址和物理地址之间的映射关系。在访问内存时，CPU使用虚拟地址，而内存管理单元（MMU）负责将虚拟地址转换为物理地址

 一个进程并非全部的页都会用到，把用到页和可能要用的页加载到内存，如果在主存未查询到需要的页，发生缺页中断，从辅助内存加载页，同时长期不用的页会被置换出去（LRU算法，FIFO算法）
...
#### （8） 死锁检测
1. 图论算法：死锁可以表示为资源分配图或等待图。通过遍历这些图，可以检测出是否存在环路。常用的图论算法包括深度优先搜索（DFS）和广度优先搜索（BFS）。

2. 资源分配表算法：通过维护一个资源分配表和进程等待表，可以检测死锁。资源分配表记录了每个进程被分配的资源，进程等待表记录了每个进程正在等待的资源。通过检查资源分配表和进程等待表之间的关系，可以判断是否存在死锁。

3. 银行家算法：银行家算法是一种死锁避免和检测的算法，常用于操作系统中。它基于资源的最大需求和可用资源的数量来检测死锁。通过模拟资源分配的过程，可以判断是否存在死锁情况。

4. 死锁检测工具：有一些工具和库可以帮助检测死锁，例如Valgrind、Lockdep（Linux内核工具）等。这些工具可以在运行时分析程序的执行和资源分配情况，并给出死锁检测报告。
#### （8） 死锁的恢复
...
- 进程标识符，唯一标识一个进程的数值
- 处理器状态， 寄存器、程序计数器等
- 进程调度信息， 包括已分配的内存地址空间、打开的文件描述符、已分配的资源等
- 进程控制信息， 进程创建时间、cpu占用时间
...
- 互斥锁（Mutex）：使用std::mutex或其派生类（如std::recursive_mutex）可以实现互斥访问共享资源。通过在访问共享资源之前获取互斥锁，在访问完成后释放互斥锁，确保同时只有一个线程能够访问共享资源。
- 条件变量（Condition Variable）：使用std::condition_variable可以实现线程之间的条件等待和通知。条件变量结合互斥锁一起使用，等待线程会在条件不满足时阻塞，并在条件满足时被唤醒。
- 原子操作（Atomic Operations）：使用std::atomic模板类可以实现对共享变量的原子操作，避免了竞争条件的问题。原子操作可以保证对共享变量的读写操作的原子性，从而避免了显式的锁和同步机制。
- 信号量（Semaphore）：虽然C++标准库本身没有提供信号量，但可以通过使用第三方库或操作系统的原生信号量来实现线程间的同步。
- 栅栏（Barrier）：使用std::barrier可以实现多个线程在某个点上进行同步，等待所有线程都到达该点后再继续执行。
...
协程允许在一个线程内实现多个独立的执行流，这些执行流可以在不同的代码块中进行切换，从而实现协作式的多任务处理
